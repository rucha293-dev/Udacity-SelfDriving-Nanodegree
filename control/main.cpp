...
int main ()
{
  ...
  pid_throttle.Init(0.2, 0.001, 0.05, 0.5, -1.0);

  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer, &prev_sim_time](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        auto s = hasData(data);
        ...
        if (s != "") {
          ...
          // Save time and compute delta time using simulator time
          time(&timer);
          ...
          for (int j = 0; j < (int)x_points.size(); j++) {
          ...
          if (desired_speed < 1.0) desired_speed = 1.0;
