...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // Use kinematics: d = v^2 / (2 * a_max) to find stopping distance
  auto look_ahead_distance = (velocity_mag * velocity_mag) / (2.0 * P_MAX_ACCEL);
  if (look_ahead_distance < 1.0) {
    look_ahead_distance = velocity_mag * P_LOOKAHEAD_TIME;
  }
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    goal = _goal;  // maintain the same stopping goal
