...
void path_planner(vector<double>& x_points, vector<double>& y_points, vector<double>& v_points, double yaw, double velocity, State goal, bool is_junction, string tl_state, vector< vector<double> >& spirals_x, vector< vector<double> >& spirals_y, vector< vector<double> >& spirals_v, vector<int>& best_spirals){
  ...
  // Generate offset goals in ego frame so the cubic spiral solver works correctly
  auto goal_set = motion_planner.generate_offset_goals_ego_frame(ego_state, goal);

  // Spirals are generated in ego frame
  auto spirals = motion_planner.generate_spirals(ego_state, goal_set);

  // Transform spirals from ego frame back to global frame for CARLA
  auto spirals_global = motion_planner.transform_spirals_to_global_frame(spirals, ego_state);

  auto desired_speed = utils::magnitude(goal.velocity);
  // If the behavior planner hasn't set a velocity yet, use the speed limit
  if (desired_speed < 0.01) {
    desired_speed = P_SPEED_LIMIT;
  }

  State lead_car_state;  // = to the vehicle ahead...

  if(spirals_global.size() == 0){
  	cout << "Error: No spirals generated " << endl;
  	return;
  }

  for(int i = 0; i < spirals_global.size(); i++){

    auto trajectory = motion_planner._velocity_profile_generator.generate_trajectory( spirals_global[i], desired_speed, ego_state,
  ...
  best_spirals = motion_planner.get_best_spiral_idx(spirals_global, obstacles, goal);
  ...
  if(best_spiral_idx < 0 || best_spiral_idx >= (int)spirals_x.size()){
    cout << "Error: No valid best spiral found" << endl;
    return;
  }
