...
class Sensor:
    ...
    def in_fov(self, x):
        ############
        # Step 4: return True if x lies in the sensor's field of view, otherwise False
        ############

        # Transform position from vehicle to sensor coordinates
        pos_veh = np.ones((4, 1))
        pos_veh[0:3] = x[0:3]
        pos_sens = self.veh_to_sens * pos_veh  # 4x1

        # Object must be in front of the sensor (positive x in sensor frame)
        if pos_sens[0] <= 0.1:
            return False

        # Compute azimuth angle in sensor frame and check against FOV limits
    ...
    def get_hx(self, x):
      ...
      if self.name == 'lidar':
        ...
        # ✅ FIX: ensure column vector (3x1 matrix)
        hx = np.matrix([
            [pos_sens[0, 0]],
            [pos_sens[1, 0]],
            [pos_sens[2, 0]]
        ...
        if pos_sens[0] == 0:
            raise NameError('h(x) not defined: object at camera plane')

        i = self.c_i - self.f_i * pos_sens[1] / pos_sens[0]
        j = self.c_j - self.f_j * pos_sens[2] / pos_sens[0]
        ...
        return hx
            ...
            # 3. Project from camera 3D coordinates to image (i, j) pixel coordinates
            # i = f_i * (y_c / x_c) + c_i
            # j = f_j * (z_c / x_c) + c_j
    ...
    def generate_measurement(self, num_frame, z, meas_list):
        ############
        # Step 4: remove restriction to lidar — include camera measurements too
        ############

        # Accept measurements from both lidar and camera
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            self.z = np.matrix(z[0:3]).reshape(3,1)
            ...
            sigma_cam_i = params.sigma_cam_i  # std dev for image i-coordinate
            sigma_cam_j = params.sigma_cam_j  # std dev for image j-coordinate

            # Measurement vector: [i, j] pixel coordinates
            self.z = np.matrix(z[0:2]).reshape(2,1)
           

            # Measurement noise covariance matrix (2x2, diagonal)
